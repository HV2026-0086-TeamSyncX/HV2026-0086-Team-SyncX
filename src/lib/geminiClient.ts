...
/**
 * Universal resilient generation trying valid active Gemini models with strict timeout
 */
export async function generateGeminiContentWithFallback(
  ...
  for (const modelName of candidateModels) {
    try {
      ...
      // Add 8-second timeout per model attempt to prevent hanging
      const generationPromise = model.generateContent(contents);
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error(`Timeout with model ${modelName}`)), 8000)
      );
      ...
      console.warn(`Gemini model ${modelName} attempt:`, err);
    }
  }
...
RULES:
1. Provide a direct, highly articulate, plain-English answer to the user's question.
2. If the user's question relates to the document, ground your response in the provided document facts and cite page numbers (e.g. *(Page 1)*).
3. If the user asks a general question, concept inquiry, or coding request not found in the document, answer it thoroughly and helpfully using your general knowledge, rather than refusing or saying it's not present.
4. Structure your answer with clear markdown headers, bold labels, bullet points, and tables where helpful.
5. NEVER say "no document text was provided", "document is blank", or ask the user to re-upload. You have the complete verified document context below.
